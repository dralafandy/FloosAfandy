...
with st.form(key="add_transaction_form"):
    ...
    # حقل إدخال الفئة الجديدة داخل النموذج
    new_category = None
    if category == "إضافة فئة جديدة":
        new_category = st.text_input("📛 أدخل اسم الفئة الجديدة", key="new_category_input")
...
if submit_button:
    try:
        # اختيار الفئة المستخدمة (موجودة أو جديدة)
        if new_category and new_category.strip():  # التأكد من أن الفئة الجديدة ليست فارغة
            fm.add_custom_category(account_id, trans_type_db, new_category)
            selected_category = new_category
        else:
            selected_category = category if category != "إضافة فئة جديدة" else "غير مصنف"  # الافتراضي إذا لم تُدخل فئة جديدة
...
if transactions:
    ...
    with st.form(key="edit_transaction_form"):
        ...
        # حقل إدخال الفئة الجديدة للتعديل
        edit_new_category = None
        if edit_category == "إضافة فئة جديدة":
            edit_new_category = st.text_input("📛 أدخل اسم الفئة الجديدة", key="edit_new_category_input")
    ...
    if save_button:
        try:
            if edit_new_category and edit_new_category.strip():
                fm.add_custom_category(edit_account, edit_type_db, edit_new_category)
                final_category = edit_new_category
            else:
                final_category = edit_category if edit_category != "إضافة فئة جديدة" else "غير مصنف"
